...
const struct pin_pair * MOTOR_A; //  = pin_pair_init(MOTOR_A_P, MOTOR_A_N);
const struct pin_pair * MOTOR_B; //  = pin_pair_init(MOTOR_B_P, MOTOR_B_N);
const struct pin_pair * MOTOR_C; //  = pin_pair_init(MOTOR_C_P, MOTOR_C_N);
const struct pin_pair * MOTOR_D; //  = pin_pair_init(MOTOR_D_P, MOTOR_D_N);
...
const struct pin_pair * ENCODER_A; // = pin_pair_init(ENCODER_A_B, ENCODER_A_A);
const struct pin_pair * ENCODER_B; // = pin_pair_init(ENCODER_B_B, ENCODER_B_A);
const struct pin_pair * ENCODER_C; // = pin_pair_init(ENCODER_C_B, ENCODER_C_A);
const struct pin_pair * ENCODER_D; // = pin_pair_init(ENCODER_D_B, ENCODER_D_A);
...
const float SHUNT_RESISTOR = 0.47f;
const float CURRENT_GAIN = 1;
const float VOLTAGE_GAIN = 3.9f / 13.9f;
const float CURRENT_OFFSET = -0.005f;
